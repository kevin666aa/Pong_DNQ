...
def compute_td_loss(model, target_model, batch_size, gamma, replay_buffer):
    ...
    target_next = target_model(next_state)
    target_next = target_next.max(1)[0]
    Q_target = reward + gamma * target_next * (1 - done)
...
class ReplayBuffer(object):
    ...
    def sample(self, batch_size):
        ...
        #------------------my code--------------------
        one_sample = random.sample(self.buffer, batch_size)
        (state, action, reward, next_state, done) = zip(*one_sample)
        #---------------------------------------------
